        ...
        const result = table.map(row => {
            ...
            for (const col of selectCols) {
                ...
                if (col === '*') {
                    ...
                    let val;
                    const coalesceMatchA = orig.match(/^COALESCE\((.+)\)$/i);
                    if (coalesceMatchA) {
                        const args = coalesceMatchA[1].split(',').map(s => s.trim());
                        for (const a of args) {
                    ...
                    const coalesceMatch = col.match(/^COALESCE\((.+)\)$/i);
                    if (coalesceMatch) {
                    ...
                    const caseMatch = col.match(/^CASE\s+WHEN\s+(.+?)\s+THEN\s+(.+?)(?:\s+ELSE\s+(.+?))?\s+END$/i);
                    if (caseMatch) {
                        const cond = caseMatch[1].trim();
                        const thenExpr = caseMatch[2].trim();
                        const elseExpr = caseMatch[3] ? caseMatch[3].trim() : null;
